...
'***********************************************************************************
'--- FIX (Win11 compat / compile error "User-defined type not defined"):
'--- all Types used below in Declare statements must be defined BEFORE those
'--- Declare lines in the same module - VB6 parses top-to-bottom for this.
'--- These were originally scattered further down in the file, after the
'--- Declares that use them, which is what caused the compile error.
...
'--- FIX (Win11 compat): 64-bit safe replacement for MEMORYSTATUS.
'--- VB6 has no native 64-bit integer, so the ull* fields use Currency (8 bytes).
'--- Convert to a real number with: CDbl(field) * 10000  (see MemBytes() helper below)
...
'--- FIX: moved here (next to the Types) so it's declared well before any
'--- Sub that uses it - this codebase has shown it's sensitive to ordering.
Private info(1 To 500) As infostructre, totalinf As Integer, Myname As String
Private CurrentProcess As infostructre

'--- FIX (Win11 compat): replaces the old HKEY_DYN_DATA-based CPU reading.
'--- HKEY_DYN_DATA is a Windows 95/98/ME-only registry hive - it has never
'--- existed on any NT-based Windows (2000/XP/Vista/7/8/10/11), so RegOpenKey
'--- against it always failed, triggering "MsgBox error" + End (crashing the
'--- whole app) every time InitCPU/Timer3_Timer ran. GetSystemTimes is the
'--- standard, always-available NT API for CPU usage.
...
'--- NEW: real RAM-freeing, replaces the old memory-ballast placebo trick.
'--- EmptyWorkingSet actually asks Windows to trim a process's working set
'--- (same thing Task Manager's "Reduce memory usage" / RAMMap do).
...
'--- FIX (Win11 compat): GlobalMemoryStatus caps every field at 4GB (0xFFFFFFFF),
'--- so on any modern PC (8/16/32GB RAM) all RAM readings below became garbage/negative.
'--- GlobalMemoryStatusEx uses 64-bit fields and works correctly regardless of RAM size.
...
'--- FIX (Win11 compat): these used to be hardcoded Consts (240x140 / 110x65),
'--- sized to match Picture1/PicCPU's pixel size at exactly 96 DPI (100% Windows
'--- scaling). At any other DPI (125%/150%/etc - very common on modern displays),
'--- the actual PictureBox is a different pixel size than these fixed numbers,
'--- so the offscreen bitmap (drawn at the old fixed size) only covered part of
'--- the real control - which is why the grid/graph didn't reach the edges.
'--- Now computed at runtime in Form_Load from the real control size.
Private CPUWidth As Long '2
Private CPUHeight As Long '2
Private CPUSchelet As Integer '2
Private Jumate As Long '2
Private pWidth As Long '1
Private pHeight As Long '1
Private pGrid As Integer '1
...
Private mBusyTimer1 As Boolean '--- FIX: re-entrancy guard, used in Timer1_Timer
Private mTick1 As Long '--- FIX: throttle counter, used in Timer1_Timer
Private mPrevIdle As Double, mPrevKernel As Double, mPrevUser As Double '--- FIX: CPU sampling
Private mHaveCPUSample As Boolean '--- FIX: CPU sampling

'--- FIX (Win11 compat): converts a Currency-encoded 64-bit value from
'--- MEMORYSTATUSEX back into a real byte count (Double, safe up to ~2^63).
'--- FIX: moved here (after all Const/Dim declarations, before the first real
'--- Sub) - VB6 requires ALL module-level declarations to come before ANY
'--- Sub/Function/Property in the module; having it in the middle of the
'--- declarations caused "Only comments may appear after End Sub/Function".
Private Function MemBytes(ByVal cur As Currency) As Double
    MemBytes = CDbl(cur) * 10000
End Function

'--- FIX (Win11 compat): FILETIME is two 32-bit halves; combine into one
'--- Double, treating each half as unsigned (Longs in VB6 are signed).
...
'--- NEW: real RAM freeing - trims the working set of every running process,
'--- the same mechanism Task Manager/RAMMap use. Some processes will fail to
'--- open (protected system processes) - that's normal, they're just skipped.
Public Function FreeAllWorkingSets() As Long
    On Error Resume Next
    Dim hSnap As Long, uP As PROCESSENTRY32, r As Long
    Dim hProc As Long, freedCount As Long
    hSnap = CreateToolhelpSnapshot(TH32CS_SNAPPROCESS, 0&)
    If hSnap = 0 Then Exit Function
    uP.dwSize = Len(uP)
    r = ProcessFirst(hSnap, uP)
    Do While r
        hProc = OpenProcess(PROCESS_QUERY_INFORMATION Or PROCESS_SET_QUOTA, 0, uP.th32ProcessID)
        If hProc <> 0 Then
            If EmptyWorkingSet(hProc) <> 0 Then freedCount = freedCount + 1
            Call CloseHandle(hProc)
        End If
        r = ProcessNext(hSnap, uP)
    Loop
    Call CloseHandle(hSnap)
    FreeAllWorkingSets = freedCount
End Function
...
Private Sub ElibereazaMem_Click()
    ...
    Timer1 = False
    TimLED.Enabled = True
    '--- NEW: real RAM freeing instead of the old memory-ballast trick.
    Dim freedCount As Long
    freedCount = FreeAllWorkingSets()
    Debug.Print "ElibereazaMem_Click: EmptyWorkingSet succeeded on "; freedCount; " processes"
...
3:
    ...
    KillProgram.Picture = PictN.Picture
    Me.Show
    Debug.Print "=== after Me.Show ==="

    '--- FIX (Win11 compat / DPI) - v2: measure the box sizes AFTER Me.Show,
    '--- not before. On Windows 11, a non-DPI-aware form's controls can get
    '--- their final on-screen pixel size only once the window actually
    '--- becomes visible (DPI virtualization applies at Show time), so
    '--- measuring earlier captured stale numbers - which is why the grid
    '--- lines were missing/wrong after the first attempt at this fix.
    Picture1.ScaleMode = 3 '3 = vbPixels
    pWidth = Picture1.ScaleWidth
    pHeight = Picture1.ScaleHeight
    PicCPU.ScaleMode = 3
    CPUWidth = PicCPU.ScaleWidth
    CPUHeight = PicCPU.ScaleHeight
    pGrid = 20
    CPUSchelet = 10
    Jumate = CPUHeight \ 2
    '--- FIX: safety fallback - if for any reason the real size couldn't be
    '--- measured yet (e.g. 0 or absurdly small), fall back to the original
    '--- hardcoded values instead of silently drawing a broken/empty graph.
    If pWidth < 10 Then pWidth = 240
    If pHeight < 10 Then pHeight = 140
    If CPUWidth < 10 Then CPUWidth = 110
    If CPUHeight < 10 Then CPUHeight = 65
    Jumate = CPUHeight \ 2
    Debug.Print "Dynamic sizes: pWidth="; pWidth; " pHeight="; pHeight; " CPUWidth="; CPUWidth; " CPUHeight="; CPUHeight; " (old hardcoded were 240x140 / 110x65)"
    ...
    '--- DEBUG: print the actual values used by the two loops below
    Debug.Print "=== Form_Load grid loops ==="
    Debug.Print "pGrid="; pGrid; " pWidth="; pWidth; " pHeight="; pHeight; " counter="; counter

    For I = pGrid To pHeight - 1 Step pGrid '1
        Debug.Print "loop1 I="; I
        Picture1.Line (0, I)-(pWidth, I), &H8000&
    Next
    Debug.Print "loop1 done, final I="; I

    For I = (pWidth - 2) Mod pGrid To pWidth - 1 Step pGrid '1
        Debug.Print "loop2 I="; I
        Picture1.Line (I, 0)-(I, pHeight), &H8000&
    Next
    Debug.Print "loop2 done, final I="; I

    For CPU = CPUSchelet To CPUHeight - 1 Step CPUSchelet '2
        PicCPU.Line (0, CPU)-(CPUWidth, CPU), &H8000&
    Next

    For CPU = (CPUWidth - 2) Mod CPUSchelet To CPUWidth - 1 Step CPUSchelet
        PicCPU.Line (CPU, 0)-(CPU, CPUHeight), &H8000&    '2
    Next

    Dim MSX As MEMORYSTATUSEX '1 -- FIX (Win11 compat)
    MSX.dwLength = Len(MSX)
    Call GlobalMemoryStatusEx(MSX)
    PaulTOT = Format$(MemBytes(MSX.ullTotalPhys) / 1024 / 1024, "#,###")
    P = InStr(PaulTOT, ",") - 1
    PaulOptim = Mid(PaulTOT, 1, P)
    '--- FIX: PaulOptim is a truncated internal scale value (kept as-is below,
    '--- the graph proportions depend on it) - it was being shown to the user
    '--- mislabeled as "Mb" when its magnitude is really closer to GB. Show a
    '--- proper GB figure instead, computed directly from the real total.
    ...
    PaulLiber = Format$(MemBytes(MSX.ullAvailPhys) / 1024 / 1024, "#,###")
    ...
    TimLED.Interval = 300
    TimLED.Enabled = False
    Debug.Print "=== Form_Load END (completed OK) ==="
End Sub
...
Private Sub Timer1_Timer()
    On Error Resume Next
    '--- FIX (possible freeze): if a previous tick is still running (GDI/BitBlt
    '--- work can be slower on modern Windows than it was on the original target
    '--- OS), skip this tick instead of letting calls pile up and choke the UI.
    If mBusyTimer1 Then
        Debug.Print "Timer1_Timer: SKIPPED (previous tick still running)"
        Exit Sub
    End If
    mBusyTimer1 = True
    Dim MSX As MEMORYSTATUSEX '-- FIX (Win11 compat)
    Dim PaulTOT, PaulLiber As Variant
    ...
    Else
        WWRRG = " b"
    End If
    '--- FIX: show real GB (with 2 decimals) instead of MB - with 64GB+ RAM,
    '--- the MB figures are unwieldy and the old scale-bar label was mislabeled.
    Label1.Caption = Format$(MemBytes(MSX.ullAvailPhys) / 1024 / 1024 / 1024, "0") & " GB"
    Label2.Caption = Format$(MemBytes(MSX.ullTotalPhys) / 1024 / 1024 / 1024, "0") & " GB"
    '--- FIX: Label9 below used to show "er" (a truncated MB value, e.g. "52")
    '--- with a hardcoded " Mb" suffix. "er" itself still drives the marker's
    '--- vertical position (left untouched), but the TEXT now shows real GB.
    Dim FreeGBDisp As String
    FreeGBDisp = Format$(MemBytes(MSX.ullAvailPhys) / 1024 / 1024 / 1024, "0") & "Gb"
    If er <> Vechi Then
        t.szTip = "Free RAM - " & FreeGBDisp & Chr$(0)
        Label9.Visible = False
        Label9.Top = Picture1.Top + (I - 5)
        Label9.Caption = "-" & FreeGBDisp
        ...
        P = InStr(PaulTOT, ",") - 1
        PaulOptim = Mid(PaulTOT, 1, P)
        '--- FIX (red/stale leftover pixels): Height used to be resized INSIDE
        '--- the drawing loop below, i.e. partly AFTER Cls already ran. If the
        '--- box grew, the newly revealed strip was never cleared that tick,
        '--- leaving stale content from a previous tick visible as a blob.
        '--- Finalize the size BEFORE clearing, so the full current area is
        '--- always wiped clean before anything is drawn.
        CompresProces.Height = Label9.Top - Label8.Top - 12
        Debug.Print "CP.Top="; CompresProces.Top; " CP.Height="; CompresProces.Height; " CP2.Top="; CompresProces2.Top; " CP2.Height="; CompresProces2.Height; " ggpp="; ggpp; " I="; I
        CompresProces.Cls
        CompresProces2.Cls
        For AI = 1 To I Step 4
            ...
            End If
        Next AI
        '--- FIX: fill the rest of the bar (below the used portion) with thick
        '--- dashed green horizontal lines all the way to the bottom, instead
        '--- of leaving it black/empty. Phase-aligned to the fill loop above
        '--- (which always starts at AI=1, step 4) so there's no seam - without
        '--- this, I+1 rarely lands on the same rhythm as 1,5,9,13...
        Dim startAI As Long
        startAI = I + 1 + (((1 - (I + 1)) Mod 4) + 4) Mod 4
        For AI = startAI To CompresProces.Height Step 4
            CompresProces.Line (0, AI)-(35, AI + 1), &H4000&, BF
        Next AI
        If ggpp Then
            Dim startAI2 As Long
            startAI2 = I + 1 + (((1 - (I + 1)) Mod 4) + 4) Mod 4
            For AI = startAI2 To CompresProces2.Height Step 4
                CompresProces2.Line (0, AI)-(35, AI + 1), &H4000&, BF
            Next AI
        End If
        '--- FIX (possible freeze / much heavier on Win11): GetBytes + RefreshProcessList
        '--- used to run on almost every timer tick (er <> Vechi is true nearly every 10ms),
        '--- and RefreshProcessList enumerates ALL running processes - Windows 11 machines
        '--- typically run several times more background processes than this app's original
        '--- target OS, making a full rebuild ~100x/second far heavier than it used to be.
        '--- Throttle it to a few times per second instead.
        mTick1 = mTick1 + 1
        If mTick1 Mod 15 = 0 Then
            GetBytes
            RefreshProcessList
        End If
    End If
    ...
    Dim bitbltResult As Long
    bitbltResult = BitBlt(hDCh, 0, 0, pWidth - 1, pHeight, hDCh, 1, 0, SRCCOPY)
    Debug.Print "shift BitBlt result="; bitbltResult; " hDCh="; hDCh; " pWidth="; pWidth; " pHeight="; pHeight; " pGrid="; pGrid; " counter="; counter

    '--- FIX (grid lines disappearing over time): the graph scrolls left every
    '--- tick (the BitBlt above), but only the vertical gridline used to get
    '--- redrawn at the new right edge. Horizontal lines, drawn once in
    '--- Form_Load, scroll off within a couple minutes and never return.
    '--- Redraw one pixel of each horizontal line per tick, same as the
    '--- vertical line already does, so they scroll back in continuously.
    Dim gy As Long
    Dim moveResult As Long, lineResult As Long
    For gy = pGrid To pHeight - 1 Step pGrid
        moveResult = MoveToEx(hDCh, pWidth - 2, gy, 0)
        lineResult = LineTo(hDCh, pWidth - 2, gy + 1)
        Debug.Print "  gridline gy="; gy; " move="; moveResult; " line="; lineResult
    Next gy
...
Private Sub InitCPU()
    '--- FIX (Win11 compat): HKEY_DYN_DATA doesn't exist on any NT-based Windows,
    '--- so this used to always fail here and crash the whole app (MsgBox + End).
    '--- Replaced with a GetSystemTimes baseline sample for the new CPU meter.
...
Private Sub Timer3_Timer()
    ...
    '--- FIX (Win11 compat): HKEY_DYN_DATA doesn't exist on any NT-based Windows,
    '--- so this used to always fail here and crash the whole app (MsgBox + End).
    '--- Replaced with a GetSystemTimes-based CPU % calculation.
    Dim ftIdle As FILETIME, ftKernel As FILETIME, ftUser As FILETIME
    ...
    If Not mHaveCPUSample Then
        ...
        mPrevUser = curUser
        mHaveCPUSample = True
        Exit Sub '--- need two samples to compute a delta; skip this first tick
    End If
    ...
    '--- FIX (grid lines disappearing over time): same issue as the RAM graph -
    '--- redraw one pixel of each horizontal line per tick so it scrolls back in.
    Dim gy2 As Long
    For gy2 = CPUSchelet To CPUHeight - 1 Step CPUSchelet
        Call MoveToEx(CPUdc, CPUWidth - 2, gy2, 0)
        Call LineTo(CPUdc, CPUWidth - 2, gy2 + 1)
    Next gy2
...
Public Sub GetBytes()
    ...
    '--- FIX (very likely cause of the freeze): the loop below used to compute its
    '--- upper bound with Int(hardTotal)/Int(HardOcupat), where hardTotal/HardOcupat
    '--- are STRINGS already formatted with thousands separators (e.g. "512,110,190,592").
    '--- Int() on a comma-formatted string silently truncates at the first comma
    '--- (Int("512,110,190,592") = 512), so on a modern multi-hundred-GB/TB Windows 11
    '--- disk the loop bound could come out wildly wrong - in some cases huge enough
    '--- that hundreds of thousands of GDI Line() calls made the app look frozen.
    '--- Fix: do the math on the raw numeric byte counts, format to text only for display.
    Dim TotalBytesNum As Double, FreeBytesNum As Double, UsedBytesNum As Double
    TotalBytesNum = CDbl(TotalBytes) * 10000
    FreeBytesNum = CDbl(BytesFreeToCalller) * 10000
    UsedBytesNum = TotalBytesNum - (CDbl(TotalFreeBytes) * 10000)

    hardTotal = Format$(TotalBytesNum, "###,###,###,##0")
    hard = Format$(FreeBytesNum, "###,###,###,##0")
    HardOcupat = Format$(UsedBytesNum, "###,###,###,##0")

    '--- FIX: show GB (2 decimals) instead of raw byte counts, which were
    '--- unreadable on modern multi-hundred-GB/TB drives (e.g. "627,837,460,480 b").
    DiskLiber.Caption = Format$(FreeBytesNum / 1024 / 1024 / 1024, "0") & " GB"

    DiskTot.Caption = Format$(TotalBytesNum / 1024 / 1024 / 1024, "0") & " GB"
    Debug.Print "GetBytes: TotalBytesNum="; TotalBytesNum; " UsedBytesNum="; UsedBytesNum; " loop bound="; ((HardLiber.ScaleWidth / TotalBytesNum) * UsedBytesNum)
    If TotalBytesNum > 0 And HardLiber.ScaleWidth > 0 Then
        For ro = 0 To ((HardLiber.ScaleWidth / TotalBytesNum) * UsedBytesNum)
            pu = (&HC000& * ro)
