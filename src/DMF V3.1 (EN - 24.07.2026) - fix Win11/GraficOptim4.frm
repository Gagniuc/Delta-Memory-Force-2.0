...
Begin VB.Form Form1 
   BackColor       =   &H00000000&
   BorderStyle     =   0  'None
   Caption         =   "Delta Memory Force 3.0 - Copyright by Paul Aurelian Gagniuc"
   ClientHeight    =   5775
   ClientLeft      =   450
   ClientTop       =   225
   ClientWidth     =   9135
   ...
   Begin VB.Label ScrieDelta 
      Caption         =   "Delta Memory Force 3.0"
      Height          =   255
      Left            =   120
      TabIndex        =   69
      Top             =   7560
...
Dim CPUHist() As Long '2 - one pixel-row value per column, oldest first, -1 = no data yet
Dim counter As Long '1
Dim oldY As Long '1
Dim hDCh As Long, hPenB As Long, hPenC As Long '1
Dim RAMHist() As Long '1 - one pixel-row value per column, oldest first, -1 = no data yet
...
Private mBusyTimer1 As Boolean
Private mTick1 As Long
...
3:
    ...
    ' On some Win11 setups (DPI scaling / slow WM_SIZE handling) Picture1 and
    ' PicCPU are not yet at their final on-screen size right after Me.Show.
    ' Reading ScaleWidth/ScaleHeight too early makes pWidth/CPUWidth smaller
    ' than the control's real width, so the grid and the offscreen buffer
    ' below only cover part of the control - the remaining strip on the
    ' right is never drawn and just shows the raw background color (the
    ' wide solid-colored block you see at startup). Pump the message queue
    ' a few times first so layout/painting has settled before we measure.
    Dim dpiPump As Integer
    For dpiPump = 1 To 15
        DoEvents
    Next dpiPump
    ...
    ' History arrays: one pixel-row value per column, oldest at index 0,
    ' newest at the end. Every tick we drop the oldest, append the newest,
    ' and redraw the whole trace from this array into the offscreen buffer.
    ' This replaces the old "shift the bitmap itself" trick entirely, so
    ' there is no BitBlt overlap and no persistent buffer content that can
    ' ever go stale or get corrupted - each frame is fully rebuilt from
    ' data we control.
    ' -1 means "no reading yet" - the chart starts empty and the trace
    ' fills in from the right as real samples arrive, instead of showing a
    ' flat line across the whole width from the very first tick.
    Dim histInit As Long
    ReDim RAMHist(0 To pWidth - 1)
    For histInit = 0 To pWidth - 1
        RAMHist(histInit) = -1
    Next histInit
    ReDim CPUHist(0 To CPUWidth - 1)
    ...
    ' Safety net: clear the whole control to black first (its real on-screen
    ' area, not just the pWidth/pHeight we measured), so if there's still any
    ' leftover strip outside the measured size it shows as plain background
    ' instead of the container's raw color.
    Picture1.Cls
    PicCPU.Cls
    ...
    t.szTip = "DeltaMemoryForce 3.0" & Chr$(0)
...
Private Sub Timer1_Timer()
    ...
    ' Redraw the RAM chart from scratch every tick, from a plain array of
    ' the last pWidth readings, instead of shifting pixels inside a bitmap.
    ' Every previous approach here (self-overlapping BitBlt, then two
    ' alternating buffers) ran into GDI/driver-specific corruption on
    ' Windows 11. An array holds normal numbers, not pixels, so there is
    ' nothing for the OS/driver to mishandle - we just draw pWidth-1 line
    ' segments into hDCh and blit the whole thing to the screen once.
    Dim k As Long
    For k = 0 To pWidth - 2
    ...
    Call PatBlt(hDCh, 0, 0, pWidth, pHeight, BLACKNESS)

    Dim gy As Long
    For gy = pGrid To pHeight - 1 Step pGrid
        Call MoveToEx(hDCh, 0, gy, 0)
        Call LineTo(hDCh, pWidth, gy)
    Next gy
    Dim gx As Long
    For gx = (pWidth - 2) Mod pGrid To pWidth - 1 Step pGrid
        Call MoveToEx(hDCh, gx, 0, 0)
        Call LineTo(hDCh, gx, pHeight)
    Next gx

    Call SelectObject(hDCh, hPenC)
    For k = 0 To pWidth - 2
        ' -1 means "no reading yet" at that column - skip it so the trace
        ' only appears where we actually have data (fills in from the
        ' right on startup instead of showing a flat line immediately).
        If RAMHist(k) <> -1 And RAMHist(k + 1) <> -1 Then
            Call MoveToEx(hDCh, k, RAMHist(k), 0)
            Call LineTo(hDCh, k + 1, RAMHist(k + 1))
        End If
    Next k
    Call SelectObject(hDCh, hPenB)
...
Private Sub Timer3_Timer()
    ...
    ' Same from-scratch redraw as the RAM chart (see Timer1_Timer) - keep a
    ' plain array of the last CPUWidth readings and redraw the whole trace
    ' every tick instead of shifting pixels inside a bitmap.
    Dim k2 As Long
    ...
    Call PatBlt(CPUdc, 0, 0, CPUWidth, CPUHeight, BLACKNESS)

    Dim gy2 As Long
    For gy2 = CPUSchelet To CPUHeight - 1 Step CPUSchelet
        Call MoveToEx(CPUdc, 0, gy2, 0)
        Call LineTo(CPUdc, CPUWidth, gy2)
    Next gy2
    Dim gx2 As Long
    For gx2 = (CPUWidth - 2) Mod CPUSchelet To CPUWidth - 1 Step CPUSchelet
        Call MoveToEx(CPUdc, gx2, 0, 0)
        Call LineTo(CPUdc, gx2, CPUHeight)
    Next gx2

    Call SelectObject(CPUdc, LinC1)
    For k2 = 0 To CPUWidth - 2
        If CPUHist(k2) <> -1 And CPUHist(k2 + 1) <> -1 Then
            Call MoveToEx(CPUdc, k2, CPUHist(k2), 0)
            Call LineTo(CPUdc, k2 + 1, CPUHist(k2 + 1))
        End If
    Next k2
    Call SelectObject(CPUdc, LinB)
