...
'--- types must be defined before the Declare statements that use them.
...
Public Sub ElibereazaMem666()
    ...
    Form1.Timer1 = False
    '--- NEW: real RAM freeing (calls the same routine as the "Free RAM" button).
    Dim freedCount As Long
    freedCount = Form1.FreeAllWorkingSets()
    Debug.Print "ElibereazaMem666 (tray icon): EmptyWorkingSet succeeded on "; freedCount; " processes"
    Form1.Timer1 = True
End Sub
